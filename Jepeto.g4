...
program {$jepetoProgram = $program.Prog}EOF;

program returns [Program Prog]:
 {
    $Prog = new Program();
    $Prog.setLine(1);
    $Prog.setMain($main.Main);
    //$Prog.setFunctions($funcdec_arr);
 }
 (funcdec_arr+=functionDeclaration)*  main  (funcdec_arr+=functionDeclaration)*;
...
main returns [MainDeclaration Main]:
{
    //$Main;
}
MAIN COLON (functionCallStatement | printStatement) ;
